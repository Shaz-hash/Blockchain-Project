...
contract rutgersDataset {
    ...
    function getPolicy() public pure returns (string memory) {{
        return "hospitalId,specialization";
    }}

    function evaluate(
        string memory datasetID,
        string memory hospitalId, string memory specialization,
        ...
        // Policy Evaluation
        bool permit = keccak256(abi.encodePacked(datasetPolicies["hospitalId"])) == keccak256(abi.encodePacked(hospitalId)) && keccak256(abi.encodePacked(datasetPolicies["specialization"])) == keccak256(abi.encodePacked(specialization));
